...
나중에는 LLM의 코드 리뷰 단계를 과감히 생략하고 엣지 케이스 변조 테스트만 집중적으로 돌린 작업도 있었다. 해 보니 변조 쪽이 리뷰어보다 싸고 자주 맞았다. (단, 변조 테스트를 돌리는 전체 범위 자체를 줄인 것이 아니라, 작성하는 변조 케이스의 개수와 검토 단계를 최적화한 것이다.)
