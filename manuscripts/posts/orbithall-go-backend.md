...
DB 접근에는 ORM을 쓰지 않고, Go 표준 `database/sql`로 SQL을 직접 썼다. 테이블이라고 해 봐야 사이트, 글, 댓글에 나중에 붙은 사용자 정도인 작은 스키마다. 여기에 ORM을 붙이는 건 배보다 배꼽이 큰 일이라고 봤다. SQL을 직접 쓰면 어떤 쿼리가 실행되는지 코드에 그대로 보이고, 코드를 생성하는 별도 단계도 필요 없다. 에이전트가 쓴 코드를 읽을 때도 실제로 어떤 쿼리가 나가는지 바로 확인할 수 있다.
